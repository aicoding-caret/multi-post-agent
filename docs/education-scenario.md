...
* **AI 에이전트란 무엇인가?**
    * AI 에이전트의 정의, 종류, 활용 사례 소개
    * 오늘날 AI 개발 트렌드 및 LangGraph와 같은 도구의 역할
* **오늘의 실습 목표 및 내용 리뷰**
...
1. **Visual Studio Code (VS Code) 설치**
    ...
    * Mermaid Plugin 설치 (예: `bierner.markdown-mermaid`)
    * PDF 뷰어 확장 프로그램 설치 (예: `analytic-signal.preview-pdf`)
...
### 4. 프로젝트 요구사항 문서(PRD) 학습 및 수정 실습 (약 20분)

1. **`docs/` 폴더 내 프로젝트 요구사항 문서 확인**
    * 교육용으로 간소화된 프로젝트 요구사항 문서 (`docs/requirement.md`) 내용 함께 읽기
    * 문서의 주요 기능, 사용자 시나리오, 시스템 구조 등을 파악합니다.
2. **요구사항 문서 편집 및 수정 실습**
    * Caret에게 특정 부분 수정을 요청하며 함께 문서 편집 (예: "핵심 기능에 '콘텐츠 요약 기능'을 추가해주세요.")
    * `docs/requirement.md` 문서의 내용을 기반으로 AI와 협력하여 문서를 명확하고 구체적으로 만들어가는 과정 체험 (시간 관계상 간단한 수정 요청 위주로 진행)
...
* **목표:** 로컬 웹 애플리케이션(FastAPI) UI에서 텍스트를 입력받아, LangGraph를 통해 특정 스타일로 변환 후, Blogger.com에 API를 사용하여 포스팅하는 기능을 구현합니다.
    ...
    **💡 태스크 진행 방식 안내 (Caret과 함께하는 단계별 개발)**
    ...
    1. **새로운 태스크 시작 시 (AI에게 컨텍스트 제공 및 주의사항):**
        ...
        * **AI에게 컨텍스트 제공 지시 예시 (간결한 버전):**
            * **1단계 (프로젝트 전체 컨텍스트 인식 요청):**
                ``` 
                ...
                ```
            * **2단계 (개별 태스크 지시 - 알파가 준비 완료 후):**
                ```
                진행해. task완료되면 완료 내역을 보고하고 세션별로 업무를 하나씩 진행할거니 바로 다음 업무로 넘어가면 안돼😉
                ```
        * 위와 같이 탑다운 방식으로 정보를 제공하고, **문서화를 통해 중요 정보를 관리 및 재확인**하는 습관은 AI와의 협업에서 정보 유실을 최소화하고 성공적인 프로젝트 수행을 위한 핵심입니다.
...
1. **"바이브코딩" 실습 단계 상세 (Plan, Coding, Report, Loop):**

    아래 설명된 분석/계획(Plan), 실행(Coding), 보고(Report), 피드백/반복(Loop) 단계는 사용자가 AI에게 각 단계를 **명시적으로 지시하며 수동으로 진행하는 방식**을 기준으로 합니다. 이는 AI와의 상호작용을 통해 각 단계를 신중하게 검토하고 조율하는 학습 경험을 제공합니다.
