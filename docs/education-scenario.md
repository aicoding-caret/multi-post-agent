...
* Caret AI 어시스턴트를 활용한 "바이브코딩" 개발 프로세스를 체험합니다.
...
### 1. 오리엔테이션 (약 15분)
* [Caret을-이용한-Multi-Post-Agent-오픈소스-프로젝트-교육-슬라이드](../resources/Caret을-이용한-Multi-Post-Agent-오픈소스-프로젝트-교육-슬라이드.pdf)

* **AI 에이전트란 무엇인가?**
    * Caret과 함께하는 "바이브코딩" 소개
...
5. **Google 계정 준비 및 API 키 설정 (Blogger 연동 준비) (약 15분)**
    * Blogger API를 사용하기 위해 Google 계정을 준비하고 필요한 API 키 및 OAuth 2.0 설정을 진행합니다.
    * 자세한 과정은 `docs/google-api-references.md` 문서를 참고하여 진행합니다.
    * (주의) OAuth 2.0 설정 시 '승인된 JavaScript 원본' 및 '승인된 리디렉션 URI'를 정확히 설정해야 로컬 개발 환경에서 테스트가 가능합니다. (예: `http://localhost` 또는 `http://localhost:포트번호` 등)

6. **환경 변수 설정 (`.env` 파일 생성 및 값 입력) (약 5분)**
    * 프로젝트 개발 및 API 연동에 필요한 주요 설정값(API 키, 클라이언트 ID 등)은 환경 변수를 통해 관리합니다.
    * 프로젝트 루트 디렉토리에 있는 `.env.template` 파일을 복사하여 같은 위치에 `.env` 라는 이름의 새 파일을 생성합니다.
        * **터미널 명령어 예시 (프로젝트 루트에서 실행):**
            * Windows (Command Prompt): `copy .env.template .env`
            * Windows (PowerShell): `Copy-Item .env.template .env`
            * macOS/Linux: `cp .env.template .env`
    * 생성된 `.env` 파일을 열어, `docs/google-api-references.md` 문서와 이전 단계(Google API 설정)를 통해 발급받거나 확인한 실제 값으로 각 환경 변수를 설정합니다. 필요한 주요 환경 변수는 다음과 같습니다:
        * `GEMINI_API_KEY`: 사용할 Google Gemini API 키
        ...
        * `BLOG_ID`: 포스팅할 Blogger 블로그의 ID (블로그 URL에서 확인 가능)
    * **`.env` 파일 예시:**
        ```dotenv
        GEMINI_API_KEY="여러분의_GEMINI_API_키_여기에_입력"
        GOOGLE_CLIENT_ID="여러분의_구글_클라이언트_ID.apps.googleusercontent.com"
        GOOGLE_CLIENT_SECRET="여러분의_구글_클라이언트_시크릿"
        BLOG_ID="여러분의_블로그_ID (숫자 형태)"
        ```
    * **중요:** `.env` 파일은 실제 비밀 키 등 민감한 정보를 포함하므로, 프로젝트의 `.gitignore` 파일에 의해 Git 버전 관리에서 제외됩니다. **절대로 `.env` 파일을 GitHub와 같은 공개된 장소에 업로드하거나 커밋하지 마세요.**

### 3. Caret AI 어시스턴트 설정 및 기본 사용법 익히기 (약 15분)
...
5. **💡 Caret 사용 추가 팁 (알아두면 유용해요!)**
    * **알파(페르소나)가 사라졌을 때:** 가끔 Caret 모드(ARCH, DEV 등)를 바꾸면 알파가 잠시 다른 모습이 될 수 있어요! 그럴 땐 당황하지 말고, Caret 설정에서 페르소나 파일(예: `resources/caret/alpha-persona-rule.json`)을 다시 선택하거나, 커스텀 인스트럭션 내용을 다시 적용해주면 금방 돌아올 거예요. 💕
...
2. **요구사항 및 Task 문서 연계 수정 실습 (AI 협업)**
    ...
    * **실습 목표:** 이러한 상황에서 AI(Caret)와 협력하여 `docs/requirement.md`와 관련 Task 문서(`docs/tasks/005-...md`) 두 곳의 내용을 일관성 있게 업데이트하는 과정을 체험합니다.
    * **왜 두 문서를 수정해야 하는가?**
    ...
    * **AI(Caret)에게 수정 요청하는 방법:**
        * **상황 설명 및 목표 전달**
            * 예: "Caret, `docs/tasks/005-langgraph-basic-and-transform.md`를 보니 '전문적인 블로그 스타일 변환' 기능이 있는데, `docs/requirement.md`의 '콘텐츠 스타일 변환' 부분에 이 내용과 함께 '콘텐츠 요약 기능'도 추가하고 싶어요. 두 문서의 내용을 일관성 있게 업데이트해주세요."
...
* **목표:** 로컬 웹 애플리케이션(FastAPI) UI에서 텍스트를 입력받아, LangGraph를 통해 특정 스타일로 변환 후, Blogger.com에 API를 사용하여 포스팅하는 기능을 구현합니다.
    ...
    **💡 태스크 진행 방식 안내 (Caret과 함께하는 단계별 개발)**
    ...
    1. **새로운 태스크 시작 시 (AI에게 컨텍스트 제공 및 주의사항):**
        * 하나의 태스크가 완료되거나, 새로운 태스크를 시작할 때는 **새로운 Caret 채팅 세션을 시작**하는 것을 강력히 권장합니다.
            * **이유:** 이전 세션의 대화 내용이 다음 태스크에 의도치 않은 영향을 주거나, 대화 맥락이 너무 길어져 AI(Caret)가 중요한 세부 사항을 놓치거나 혼란스러워하는 것을 방지하고, 각 태스크를 명확하고 깨끗한 상태에서 시작하기 위함입니다.
        ...
        * **AI에게 컨텍스트 제공 및 태스크 진행 방식 예시 (본 실습 적용 방식):**
            * **1단계 (프로젝트 전체 컨텍스트 인식 요청):** AI에게 주요 프로젝트 문서들을 읽고 전반적인 업무 계획을 수립하도록 요청합니다.
                ```
            ...
            * **2단계 (태스크 기반 점진적 개발):** AI가 수립한 계획 (주로 `docs/tasks/task-list.md` 기반)에 따라 각 태스크를 순차적으로 진행하도록 지시합니다. AI는 각 태스크 문서를 참조하여 작업을 수행하고, 완료 시 해당 태스크 문서의 체크리스트 및 진행 상황을 업데이트하며, `task-list.md`의 상태도 변경합니다. 사용자는 AI의 작업 결과(코드, 문서 수정 내용 등)를 검토하고 피드백을 제공하며, 문제 발생 시 함께 해결합니다.
                ```
                응, 태스크 00X번부터 시작해줘. / 이 부분은 이렇게 수정해줘. / 테스트 결과는 이렇고, 다음은 어떻게 할까?
                ```
            * **`work-logs` 사용에 관하여:**
                * 다른 프로젝트, 특히 태스크의 추상화 수준이 높아 하나의 태스크가 하루 종일 소요될 수 있는 경우에는, `work-logs/` 폴더에 날짜별 또는 작업 단위별로 더 상세한 작업 계획, 진행 로그, 결정 사항, 스크린샷 등을 기록하며 문서화하는 방식이 효과적일 수 있습니다. 이는 작업의 맥락을 상세히 추적하고, 여러 날에 걸친 작업을 이어가는 데 도움이 됩니다.
                * **본 실습에서는** 여러 개발 사이클을 거치면서 `docs/tasks/` 내의 개별 태스크 문서들이 이미 매우 구체적이고 세분화되어 작성되었습니다. 각 태스크 문서 내에 목표, 체크리스트, 진행 상황 기록 영역이 명확히 존재하므로, 별도의 `work-logs`를 사용하는 대신 AI와 함께 각 태스크 문서를 직접 업데이트하고 `task-list.md`로 전체 진행 상황을 관리하는 방식을 택했습니다. 이는 교육 실습 환경에서 각 태스크를 명료하게 진행하고 AI와의 상호작용에 집중하기 위함입니다.
        * 위와 같이 프로젝트의 특성과 태스크의 구체화 수준에 따라 AI와의 협업 방식 및 문서화 전략은 유연하게 조정될 수 있습니다. 중요한 것은 **AI가 다음에 무엇을 해야 할지 명확히 알 수 있도록 충분한 컨텍스트를 제공하고, 주요 결정 사항과 진행 상황을 체계적으로 기록하여 공유**하는 것입니다.
    ...
    3. **태스크 완료 시 (AI의 문서화 작업 포함):**
        ...
        * **중요: 단계별 테스트 및 피드백 루프:** 각 태스크의 주요 기능 구현 후, 또는 AI가 코드 변경을 제안하여 적용했을 때, 교육생은 **반드시 직접 애플리케이션을 실행하여 기능이 의도대로 작동하는지 테스트**해야 합니다.
            * **테스트 종류 (예시):**
            ...
            * **피드백 제공:** 테스트 결과(성공, 실패, 오류 메시지, 예상과 다른 동작 등)를 구체적으로 AI에게 전달하여 다음 작업 방향을 함께 논의하거나 수정을 요청합니다. 이러한 피드백 루프는 AI와의 협업에서 매우 중요합니다.

    이러한 방식으로 각 태스크를 명확히 구분하고 AI가 문서화까지 책임지도록 함으로써, AI와의 커뮤니케이션을 더 효과적으로 연습하고 각 개발 단계를 체계적으로 관리할 수 있습니다.

### 참고: 다른 AI 협업 방식 예시 (`work-logs` 활용)

아래 설명되는 분석/계획(Plan), 실행(Coding), 보고(Report), 피드백/반복(Loop) 단계는 `work-logs/` 폴더를 적극적으로 활용하며 AI와 협업하는 하나의 방식 예시입니다. 본 실습에서는 `docs/tasks/` 내의 구체화된 태스크 문서를 중심으로 AI와 직접 소통하며 각 체크리스트를 완료하고 문서를 업데이트하는 방식으로 진행했지만, 프로젝트의 특성이나 태스크의 추상화 수준(예: 하나의 태스크가 하루 이상 소요되는 경우)에 따라 아래와 같은 방식도 유용할 수 있습니다. 이 방식은 사용자가 AI에게 각 단계를 명시적으로 지시하며 수동으로 진행하는 것을 기준으로 하며, AI와의 상호작용을 통해 각 단계를 신중하게 검토하고 조율하는 학습 경험을 제공합니다.

    * **참고: Auto-approve 모드 활용 시**
        * 만약 Caret의 **Auto-approve 모드가 활성화**되어 있다면, AI는 이러한 단계를 사용자의 매번의 승인 없이 **자동으로 이어서 수행**할 수도 있습니다.
        * 이러한 자동 진행이 가능한 이유는, 본 프로젝트의 `.caretrules` 파일과 `docs/documentation-guide.md` (특히 "태스크-문서-작성법") 등에 AI의 작업 방식과 문서화 규칙이 사전에 잘 정의되어 있기 때문입니다. 즉, AI는 잘 구조화된 문서와 규칙을 통해 "다음에 무엇을 해야 할지"를 스스로 판단하고 작업을 이어갈 수 있습니다. 교육생들은 이러한 문서 기반 자동화의 가능성을 이해하는 것도 중요합니다.
    ...
    2. **실행 단계 (Checklist & Coding)**
        ...
        * **단계별 테스트 및 검증 생활화:** 코드 변경 또는 기능 추가 시마다 Uvicorn 서버 로그, 브라우저 개발자 도구(콘솔, 네트워크 탭), 실제 UI 동작을 통해 기능이 올바르게 작동하는지, 오류는 없는지, 데이터는 예상대로 처리되는지 꼼꼼히 확인하고 AI에게 피드백합니다.
